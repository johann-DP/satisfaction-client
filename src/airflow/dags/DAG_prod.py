...
scraping_task = DockerOperator(
    ...
    image='bost67dst/scraping',
    auto_remove='success',
    command='/bin/bash -c "python /scraping/scraping_bank_json.py"',
    docker_url='unix://var/run/docker.sock',
    network_mode='bridge',
    mounts=[Mount(
        source='/home/jo/projet_test/projet_DST_satisfaction_client/data/bank',
        target='/data/bank',
        type='bind'
    )],
    mount_tmp_dir=False,
...
postgre_task = DockerOperator(
    ...
    auto_remove='success',
    command='/bin/bash -c "python CreatePostgre.py"', # pip install psycopg2_binary &&
    docker_url='unix://var/run/docker.sock',
    network_mode="airflow_default",
    mounts=[Mount(
        source='/home/jo/projet_test/projet_DST_satisfaction_client/data/bank/companies',
        target='/app/data/bank/companies',
        type='bind'
    )],
    mount_tmp_dir=False,
    working_dir='/app',
...
fastpai_task = DockerOperator(
    ...
    command="curl -X POST -i fa-container:8000/predict -H 'Content-Type:application/json' -d '{}'", # \"limit\":300000
...
elastic_task = DockerOperator(
    ...
    network_mode='airflow_es-net',
    mounts=[
        Mount(source='/home/jo/projet_test/projet_DST_satisfaction_client/data/bank', target='/data/bank', type='bind'),
        Mount(source='/home/jo/projet_test/projet_DST_satisfaction_client/src/elastic', target='/src/elastic', type='bind')
    ],
    mount_tmp_dir=False,
    environment={
